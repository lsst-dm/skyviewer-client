...
| `tickets/DM-55906`                  | fork `main`     | the usdfprod standup, released as `v2026.08.24.1`: `BASE_URL` read at runtime instead of the build-time `NEXT_PUBLIC_BASE_URL`, so one image serves both RSPs. Paired with `tickets/DM-55906` in `lsst-sqre/phalanx`                                                                                           |
