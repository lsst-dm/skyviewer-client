...
- `.prettierrc.json` is an empty file — all defaults. eslint extends
  ...
  unused imports are errors; `exhaustive-deps` is a warning. **Known
  breakage:** eslint currently aborts with `couldn't determine the plugin
  "import" uniquely` — two copies of `eslint-plugin-import` (top-level via
  `eslint-config-standard` vs nested under `eslint-config-next`), so
  `yarn lint:js`'s eslint half fails before linting anything. Also note
  upstream `main` carries pre-existing prettier violations in files the
  lint script's `*.{js,jsx}` glob never covers (e.g. Search,
  SonificationControls) — don't "fix" them in unrelated diffs.
