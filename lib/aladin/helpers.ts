...
export const tameWheelZoom = (aladin: Aladin) => {
  ...
  view.throttledTouchPadZoom = () => {
    ...
    // Track the view's limits so an exhausted gesture does not keep winding
    // up a phantom zoom that must be unwound before the direction reverses
    if (view.minFoV) {
      target = Math.max(target, view.minFoV);
    }
    if (view.maxFoV) {
      target = Math.min(target, view.maxFoV);
    }
